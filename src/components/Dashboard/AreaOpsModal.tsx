...
export const AreaOpsModal: React.FC<AreaOpsModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 sm:p-6 animate-in fade-in duration-200">
      <div className="bg-white dark:bg-[#0c1017] border border-zinc-200 dark:border-zinc-800 rounded-3xl max-w-4xl w-full max-h-[90vh] flex flex-col shadow-2xl dark:shadow-[0_4px_30px_rgba(255,255,255,0.08)] overflow-hidden text-zinc-950 dark:text-white font-sans select-none">
        
        {/* Header */}
        <div className="p-6 border-b border-zinc-200 dark:border-zinc-800 flex items-start justify-between gap-4 bg-zinc-50 dark:bg-zinc-900/80">
          <div>
            <div className="flex items-center gap-2 mb-1">
              <span className="text-[10px] font-mono font-bold px-2.5 py-0.5 rounded-full border bg-zinc-200 dark:bg-zinc-950 text-zinc-700 dark:text-zinc-300 border-zinc-300 dark:border-zinc-700">
            ...
            <h2 className="text-xl font-black brand-title text-zinc-950 dark:text-white">
              {areaInfo.title}
            </h2>
          ...
          <button
            onClick={onClose}
            className="text-zinc-600 hover:text-zinc-950 dark:text-zinc-400 dark:hover:text-white p-2 rounded-xl hover:bg-zinc-200 dark:hover:bg-zinc-800 transition cursor-pointer"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="p-4 border-b border-zinc-200 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-900/50">
          <div className="relative">
            <input
              ...
              className="w-full bg-white dark:bg-zinc-950 border border-zinc-300 dark:border-zinc-800 rounded-2xl pl-9 pr-4 py-2.5 text-xs text-zinc-950 dark:text-white placeholder-zinc-400 focus:outline-none focus:border-amber-500 transition"
            />
        ...
        <div className="p-6 overflow-y-auto space-y-3 custom-scroll flex-1 bg-transparent">
          {filtered.length === 0 ? (
            ...
            filtered.map(item => (
              <div
                key={item.id}
                className={`bg-zinc-50/80 dark:bg-zinc-900/90 border rounded-2xl p-4.5 transition flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 shadow-sm ${
              ...
              >
                ...
                <div className="space-y-1.5 flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className="text-base font-black font-mono text-zinc-950 dark:text-white">{item.op}</span>
                    <span className="text-xs font-bold text-zinc-500 dark:text-zinc-400 font-mono">REF: {item.referencia}</span>
                    
                    <span className="bg-zinc-200 dark:bg-zinc-950 text-zinc-700 dark:text-zinc-300 text-[10px] font-mono font-bold px-2 py-0.5 rounded border border-zinc-300 dark:border-zinc-800">
                      {item.estado.replace('_', '-')}
                    </span>

                    {item.tieneRetraso && (
                      <span className="bg-rose-50 dark:bg-rose-950/80 text-rose-700 dark:text-rose-300 text-[10px] font-mono font-bold px-2 py-0.5 rounded border border-rose-200 dark:border-rose-800 flex items-center gap-1">
                  ...
                  <div className="text-xs text-zinc-800 dark:text-zinc-200 font-semibold font-mono">
                    <span>{item.tela}</span> • <span>{item.codigoMt}</span> • <span className="opacity-70">{item.color}</span>
                  </div>

                  <div className="text-[11px] text-zinc-600 dark:text-zinc-400 flex items-center gap-3 font-mono">
                    <span>Rollos: <strong className="text-zinc-950 dark:text-white">{item.rollos}</strong></span>
                    <span>Lote: <strong className="text-zinc-950 dark:text-white">{item.lote}</strong></span>
                    <span>Registró: <strong className="text-zinc-950 dark:text-white">{item.inspector}</strong></span>
                ...
                {/* Right: Actions */}
                <div className="flex items-center gap-2 w-full sm:w-auto justify-end border-t sm:border-t-0 border-zinc-200 dark:border-zinc-800 pt-3 sm:pt-0">
                  <button
                    onClick={() => onViewDetail(item)}
                    className="px-3.5 py-2 rounded-xl bg-zinc-100 dark:bg-zinc-800 hover:bg-zinc-200 dark:hover:bg-zinc-700 text-zinc-900 dark:text-white text-xs font-bold flex items-center gap-1.5 transition cursor-pointer border border-zinc-300 dark:border-zinc-700"
                  >
        ...
        {/* Footer */}
        <div className="p-4 border-t border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/80 flex items-center justify-between text-xs text-zinc-600 dark:text-zinc-400 font-mono">
          <span>Mostrando {filtered.length} de {areaItems.length} órdenes</span>
          <button
            onClick={onClose}
            className="px-5 py-2 bg-zinc-950 text-white hover:bg-zinc-800 dark:bg-white dark:text-zinc-950 dark:hover:bg-zinc-200 rounded-xl font-bold transition cursor-pointer"
          >
            Cerrar
          </button>
        </div>
